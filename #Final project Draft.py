...
def framsRead(seqs,minORF,n): #Define a function with argument sequences, minORF, and a intege to perfrom the ORF 1~3 reading and return the ORF sequences and their positions.
    complement = {'A': 'T', 'C': 'G', 'G': 'C', 'T': 'A'}
    framL = []
    pos = []
    for dna in seqs:
        if n > 3:
            dna = ''.join(complement[base] for base in reversed(dna))
            n = n-3
        allFram = []
        fram = ""
        starts = False
